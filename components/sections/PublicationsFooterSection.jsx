  ...
  useEffect(() => {
    ...
    function setImageLeft() {
      ...
      if (imageWrapRef.current) {
        gsap.set(imageWrapRef.current, { width: window.innerWidth, x: 0, opacity: 1 })
      }
    }
    ...
    function onScroll() {
      ...
      // Mobile: always visible | Desktop: p 0.75 → 1.0
      if (isMobile) {
  ...
  return (
    <div ref={wrapperRef} className={styles.wrapper}>
      <div ref={stickyRef} className={styles.sticky}>
        ...
        <video ref={videoSrcRef} muted playsInline className={styles.hiddenVideo} />

        {/* ── Mobile background image (footer phase - mobile only) ── */}
        <div className={styles.mobileFooterBg}>
          <Image
            src="/assets/mobile-footer-v2.jpg"
            alt=""
            fill
            quality={100}
        ...
        <div ref={imageWrapRef} className={styles.imageWrap}>
          <Image
            src="/assets/durga-footer-v4.jpg"
            alt=""
            fill
            quality={100}
        ...
        <div ref={footerContentRef} className={styles.footerContent}>
          ...
          {/* ── Mobile: redesigned minimal footer ── */}
          <div className={styles.mobileLayout}>
            {/* Logo */}
            <div className={styles.mobileLogo}>
              <span className={styles.mobileLogoText}>KDM</span>
              <span className={styles.mobileLogoDot} />
            </div>

            {/* Tagline */}
            <p className={styles.mobileTagline}>Designing Digital Experiences</p>

            {/* Social Icons Row */}
            <div className={styles.mobileSocialsRow}>
              {profile.socials.map((s) => {
                const Icon = MOBILE_SOCIAL_ICONS[s.label]
                if (!Icon) return null
                return (
                  <a
                    key={s.label}
            ...
            <p className={styles.mobileCopyright}>
              &copy; {year} {profile.name.full}. All rights reserved.
            </p>
          </div>
                  ...
                  {profile.socials.slice(0, 4).map((s, i) => (
                    <span key={s.label} className={styles.footerLinkWrap}>
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className={`${styles.footerLink} ${styles[s.label.toLowerCase()]}`}
                      >
          ...
          <div ref={bottomBarRef} className={styles.bottomBar}>
            <div className={styles.bottomLeft}>
              <div className={styles.monogram}>
                <span className={styles.monoLetters}>KM</span>
